...
class Pos extends MY_Controller
{
    ...
    public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $repair_status=0;
            if($this->input->post('type_h') ==='Repair') $repair_status=1;
            $data = array('date'  => $date,
                ...
                'type'              => $this->input->post('type_h'),
                'type_detail'       => $this->input->post('type_detail_h'),
                'repair_note'       => $this->input->post('repair_note_h'),
                'tech_id'           => $this->input->post('tech_id_h'),
    ...
    public function close_register($user_id = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['dcsales'] = $this->pos_model->getRegisterDebitSales($register_open_time, $user_id);
    ...
    public function register_details()
    {
        ...
        $this->data['dcsales'] = $this->pos_model->getRegisterDebitSales($register_open_time);
    ...
    public function today_sale()
    {
        ...
        $this->data['dcsales'] = $this->pos_model->getTodayDebitSales();
    ...
    public function update_product()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect(isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : 'pos');
        }

        if ($this->form_validation->run() == true && $this->pos_model->addProduct($data)) {
//        if ($this->form_validation->run() == true && $this->sales_model->addProduct($id, $status, $note)) {
            $this->session->set_flashdata('message', lang('Product_Saved'));
            admin_redirect(isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : 'pos');
        } else {
            $this->data['categories'] = $this->site->getAllCategories();
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme.'pos/update_product', $this->data);
